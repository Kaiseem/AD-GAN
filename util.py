...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.iter_count - opt.n_iters) / float(opt.n_iters_decay + 1)
            return lr_l
...
class data_prefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            self.next_samples = to_cuda(self.next_samples, self.device)
    ...
    def next(self):
        if self.prefetch:
            ...
            try:
                ...
                samples= to_cuda(samples, self.device)
            except StopIteration:
                samples = None
        return samples